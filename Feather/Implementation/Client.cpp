...
bigint** Client::blind_shuffled_bl_(bigint** s_bl, int table_size, uint8_t* key, uint8_t* iv, int key_size, int num_of_PRNs_, int byte_, bigint pubmoduli_){
...
bigint** Client::gen_map_(int size, uint8_t* label_key1, uint8_t* label_iv1, uint8_t* label_key2, uint8_t* label_iv2, int byte_){
...
int Client::gen_binIndx(bigint elem, int table_size){
	...
	uint8_t digest[CryptoPP::SHA512::DIGESTSIZE];
	hash2.CalculateDigest(digest, (uint8_t*)s_val.c_str(), nDataLen); // hashes the element.
	s_val.clear();
  	mpz_init(b);
...
CompPerm_Request * Client::gen_compPerm_req(uint8_t (& tmp_key_)[AES::DEFAULT_KEYLENGTH], uint8_t (& tmp_iv_)[AES::BLOCKSIZE]){
	
	AutoSeededRandomPool prng;
  	uint8_t temp_key[AES::DEFAULT_KEYLENGTH];
  	uint8_t temp_iv[AES::BLOCKSIZE];
...
bigint* Client::blind_BFs_(bigint* bf, int bf_size, uint8_t* BF_key, uint8_t* BF_iv, bigint pr_moduli){
...
bigint* Client::gen_BF_PRN_(int indx, int counter_indx, uint8_t* BF_key, uint8_t* BF_iv){
...
bigint* Client::unblind_BFs_(bigint* BF, int bf_size, uint8_t* BF_key, uint8_t* BF_iv, bigint pr_moduli){
...
//**********************************************************************
// - Function description: unblinds a biginteger representing a Bloom filters.
bigint* Client::unblind_BF_(bigint BF, int  indx, uint8_t* BF_key, uint8_t* BF_iv, bigint pr_moduli){
...
bigint* Client::blind_BF_(bigint bf, int indx, uint8_t* BF_key, uint8_t* BF_iv, bigint pr_moduli){
